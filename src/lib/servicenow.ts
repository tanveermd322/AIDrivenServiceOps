...
// ── Demo / mock data ────────────────────────────────────────────────────
// The `samples.servicenow` schema does not exist in your Databricks instance.
// The dashboard now renders with realistic demo data so you can explore the UI.
// When you create the real table, set USE_LIVE_DATA = true (or remove the flag).
...
const DEMO: Record<QueryKey, any[]> = {
  incident_kpis: [
...
export async function fetchServiceNow<T = Record<string, any>>(key: QueryKey) {
  if (USE_LIVE_DATA) return fetchLive<T>(key);
